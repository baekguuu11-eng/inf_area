...
public class EnemyMeleeAI : MonoBehaviour
{
    ...
    // [버그 방지용 자동 점검] Attack Hit Radius가 Attack Range보다 작으면
    // 공격 상태에 들어간 순간 이미 판정 밖일 수 있어 거의 항상 빗나가게 됨.
    ...
    private void FixedUpdate()
    {
        ...
        // 박치기 진행 중(튀어나가는 중/복귀 중)에는 코루틴이 직접 위치를 제어하므로 관여하지 않음
    ...
    // 박치기: 목표 지점(chargeDistance만큼 떨어진 곳)까지 짧게 튀어나갔다가,
    // 도중에 맞든 안 맞든 항상 원래 위치로 부드럽게 되돌아옴.
    private IEnumerator ChargeAttackRoutine()
    {
        ...
        // 1) 목표 지점까지 튀어나가는 구간 (플레이어/벽에 맞으면 HandleChargeContact가 isCharging을 꺼서 즉시 중단)
        float elapsed = 0f;
    ...
    // 박치기 도중 실제 몸통(Collider)이 플레이어나 벽에 부딪혔을 때 처리.
    // Is Trigger 설정 여부에 상관없이 감지되도록 Collision/Trigger 둘 다 대응함.
    ...
    private void HandleChargeContact(GameObject other)
    {
        ...
        if (IsObstacle(other))
        {
            isCharging = false; // 벽에 닿으면 그 자리에서 바로 복귀 시작
            return;
        }
        ...
        if (other.CompareTag("Player"))
        {
            ...
            isCharging = false; // 명중한 자리에서 바로 복귀 시작
